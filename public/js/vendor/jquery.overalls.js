...
(function($) {
  ...
  $.extend(Overalls.prototype, {
    ...
    display: function(html, callback) {
      ...
      $('#overalls-container, #overalls-overlay, #overalls-transparency').live('click.overalls', function() { 
        if($(this).attr('id')!='overalls-overlay'){
          self.close();
        }  
      });
    ...
    setContent: function(html) {
      ...
      if(!container.length) {
        container = $('<div id="overalls-container"></div>').css({
          ...
          left: 0,
          width: '100%',
          // height: '0px', //fucking ie doesn't like this
          opacity: 0.0,
  ...
  $.overalls = function(html, opts, callback) {
    ...
    if(!overalls){ 
      overalls = $.overalls.instance = new Overalls(); 
    }
